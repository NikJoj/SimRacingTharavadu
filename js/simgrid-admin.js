...
async function previewRaceResult() {
  ...
  try {
    ...
    openSyncPreviewModal('Preview League Race Result', `
      ...
      ${p.result.Result.map(row => `<tr><td>${row.Position}</td><td>${escapeHtml(row.DriverName)}</td><td>${escapeHtml(row.CarNumber)}</td><td>${escapeHtml(row.CarClass)}</td><td>${row.NumLaps}</td><td>${formatPreviewMs(row.BestLap)}</td><td>${escapeHtml(row.FinishStatus)}</td></tr>`).join('')}</tbody></table></div>
